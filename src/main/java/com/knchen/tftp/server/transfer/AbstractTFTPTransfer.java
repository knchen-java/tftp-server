 ...
 * @since 2021/10/31
 */
public abstract class AbstractTFTPTransfer<open extends TFTPRequestPacket, receive extends TFTPPacket, send extends TFTPPacket> implements TFTPTransfer<open, receive, send> {
    ...
    public synchronized void timeout() {
        ...
        if (++retryTimes > TFTPTransfer.MAX_RETRY_TIMES) {
            close();
            throw new TFTPTransferException(String.format("%s timeout more than %d times, fileName=%s, current blockNumber=%d",
    ...
    @Override
    public void close() {
        LOG.debug("{} {}, fileName={}, current blockNumber={}", this.getClass().getSimpleName(), complete, fileName, blockNumber);
        channel.close();
    }
